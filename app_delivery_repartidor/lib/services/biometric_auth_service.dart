...
/// éxito, cancelación del repartidor (no es un error, no se muestra nada) y
/// error real (se muestra el mensaje). El [CourierModel] viaja de vuelta porque
/// el login necesita su `status` para decidir entre /home y /review.
class BiometricSignInResult {
  ...
  const BiometricSignInResult.success(CourierModel courier)
      : this._(courier: courier);
  ...
  bool get isSuccess => courier != null;
}

/// Acceso rápido con huella / Face ID.
///
/// No reemplaza al login por correo y contraseña: lo complementa. Tras un login
/// exitoso el repartidor puede activar esta opción desde Seguridad y privacidad,
/// y a partir de ahí las credenciales quedan cifradas en el almacén seguro del
/// sistema operativo (Android Keystore / iOS Keychain) — nunca en Firestore,
...
class BiometricAuthService {
  ...
  ///
  /// Devuelve true si se verificó, false si el repartidor canceló o no coincidió.
  /// Lanza [BiometricUnavailableException] si el problema es de configuración
  /// del dispositivo y vale la pena explicárselo.
  static Future<bool> authenticate({String? reason}) async {
    try {
      return await _localAuth.authenticate(
        localizedReason:
            reason ?? 'Verifica tu identidad para ingresar a Dely Repartidor',
        biometricOnly: true,
      );
  ...
  /// mecanismo que ya usa [AuthService.changePassword] — para no llegar a
  /// persistir una contraseña equivocada que luego rompería el ingreso con
  /// huella.
  ///
  ...
  /// y si el toggle de Seguridad aparece activado.
  ...
  static Future<BiometricSignInResult> signInWithBiometrics() async {
    ...
    try {
      final courier = await AuthService.signIn(email, password);
      if (courier == null) {
      ...
      return BiometricSignInResult.success(courier);
...
/// listo para mostrarle al repartidor.
